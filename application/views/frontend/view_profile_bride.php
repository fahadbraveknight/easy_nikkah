...
<!-- 				<ul class="login_details">
			...
			</div> <div class="profile-parent inline-block">
			    <?php if($this->session->userdata('userid') != $bride['id']){ ?>
				    ...
				    <?php if(!empty($data_status)){ ?>
						   <input  class="<?php echo $data_status=='awaiting_response'? 'hvr-wobble-vertical' : ''; ?> rel-button btn_1" data-status="<?php echo $data_status ?>" data-id="<?php echo $bride['id'] ?>" type="button" value="<?php echo $data_value ?>">
						   <?php echo $button ?>
					<?php } ?>
					</div>
				<?php } ?>
